...
![Version](https://img.shields.io/badge/version-v1.3.1-2ea44f)
...
- **网页登录控制台**：密码登录后查看流量趋势、ECS、账单、计划和日志，可手动开关机、暂停恢复、立即检测并修改计划与全局设置。
...
14) 查看最近日志
15) 重启后台服务
16) 更新 GitHub 版本  [有新版本 v1.3.2]  # 仅发现更新时显示提示
17) 退出
```

面板标题始终显示当前版本号，例如 `阿里云保活与通知 v1.3.1 - 管理面板`。发现更新时，标题下方和第 16 项会显示黄色的新版本提示；启动检查超时或 GitHub 暂时不可用不会阻塞其他管理操作，也不会自动安装更新。设置 `NO_COLOR=1` 或将输出重定向到文件时，提示会自动退回纯文本。
...
网页面板提供实例总览、CDT 使用率与趋势、ECS 状态、BSS 账单、每日计划、下一动作、手动开关机、暂停恢复、立即检测、日志和全局设置。桌面使用双列实例视图，手机自动切换为单列布局。
...
也可以在设置中选择监听 `0.0.0.0`。保存后程序会自动检测本机出口 IPv4，并直接显示完整浏览器地址；检测不到时才显示 `http://服务器IP:8765`。公网监听必须同时配置防火墙白名单，推荐使用 Nginx/Caddy HTTPS 反向代理，并在面板设置中启用 `Secure Cookie`；不要通过公网明文 HTTP 输入登录密码。

安全机制包括：

- HttpOnly、SameSite 严格 Cookie 和 12 小时会话。
- 所有写操作进行 CSRF 校验。
- 单个来源 5 分钟最多连续登录失败 5 次。
- API 只返回脱敏 AccessKey，不返回 AccessKey Secret、Bot Token、代理凭据或节点链接。
...
更新确认页面会同时显示当前版本和 GitHub 最新版本；例如从 `v1.3.0` 更新时会显示 `最新版本: v1.3.1`，确认提示中也会带上目标版本号。如果构建指纹一致，则显示 `当前版本已经是最新版本了。` 并直接返回，不重复下载安装。
...
| 输入正确密码后仍提示“请先登录” | 通过 HTTP 访问，但网页设置启用了 Secure Cookie | 进入菜单第 10 项，将 HTTPS 选项改为 `N`；已有 HTTPS 反向代理时改用 HTTPS 地址 |
