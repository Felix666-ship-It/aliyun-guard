...
![Version](https://img.shields.io/badge/version-v1.3.0-2ea44f)
...
14) 查看最近日志
15) 重启后台服务
16) 更新 GitHub 版本  [有新版本 v1.3.1]  # 仅发现更新时显示提示
17) 退出
```

面板标题始终显示当前版本号，例如 `阿里云保活与通知 v1.3.0 - 管理面板`。发现更新时，标题下方和第 16 项会显示黄色的新版本提示；启动检查超时或 GitHub 暂时不可用不会阻塞其他管理操作，也不会自动安装更新。设置 `NO_COLOR=1` 或将输出重定向到文件时，提示会自动退回纯文本。
...
也可以在设置中选择监听 `0.0.0.0`，然后访问 `http://服务器IP:8765`。公网监听必须同时配置防火墙白名单，推荐使用 Nginx/Caddy HTTPS 反向代理，并在面板设置中启用 `Secure Cookie`；不要通过公网明文 HTTP 输入登录密码。
...
更新确认页面会同时显示当前版本和 GitHub 最新版本；例如从 `v1.2.8` 更新时会显示 `最新版本: v1.3.0`，确认提示中也会带上目标版本号。如果构建指纹一致，则显示 `当前版本已经是最新版本了。` 并直接返回，不重复下载安装。
